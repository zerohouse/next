...
public class LetterController {
	...
	public void update(Http http, DAO dao) throws JsonAlert {
		...
		int count = Integer.parseInt(dao.getRecord("SELECT COUNT(Letter_id) from Letter WHERE Letter_key=? and Letter_email=?", 1, letter.getKey(),
				letter.getEmail()).get(0).toString());
		if (count > 3)
