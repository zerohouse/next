...
app.controller('loveTypeController', ['$http', '$scope', function ($http, $scope) {
    ...
    $scope.result.R.head = "정열적인 사랑(Passionate Love)";
    $scope.result.R.body = "좋아하는 사람을 만나면 '첫눈에 반했다'는 생각이 들고 두 사람의 관계가 순식간에 가까워진다면 정열적인 사랑을 추구하는 사람이다. 이런 부류의 사람들은 상대방의 외모(부분적이든,전반적이든)에 많은 비중을 두는 경향이 있다. 자신의 속마음을 쉽게 털어놓고, 상대방에 대해서도 속속들이 알고 싶어한다. 남의 눈을 별로 의식하지 않고 신체적인 접촉을 즐기거나 상대방을 이상화시켜 과대평가하는 경향이 심하다. 첫눈에 황홀감을 경험하는 것만큼 헤어져야 하는 상황에서는 심한 절망감을 경험한다.";
    $scope.result.B = {};
    $scope.result.B.max = 8;
    $scope.result.B.head = "친구같은 사랑(Friendship Love)";
    $scope.result.B.body = "동료나 이성친구로 오랫동안 함께 지내는 사람에게 자기도 모르게 서서히 사랑하는 감정이 생기는 경우이다. 여기에 속하는 사람들은 상대에게 첫눈에 반해 황홀감을 느끼진 않지만 하는 일이나 취미가 비슷하고 허물없이 지내는 사람에게서 외모보다는 상호간의 신뢰감을 기반으로 사랑의 감정을 싹틔운다. 서로 잘 알고 정서적으로 안정되어 있기 때문에 서로 떨어져 있어도 초조해 하지 않으며 애정표현이 유난스럽지 않다. 비교적 덤덤한 관계를 유지하나, 결혼을 한 경우 이혼율은 낮은 편이다.";
    $scope.result.F = {};
    $scope.result.F.max = 9;
    $scope.result.F.head = "소유적 사랑(Possessive Love)";
    $scope.result.F.body = "상대방을 완전하게 소유하는 것을 사랑이라고 생각하기 때문에 상대에게 매우 헌신적이며, 상대에게도 이를 요구한다. 상대에 대한 헌신에 보답을 받지 못하거나 배신당했을 때는 강한 분노감을 나타낸다. 사랑하고 사랑받는 것에 의해 자신의 존재가 확인되는 경향이 있기 때문에 상대에게 '사랑해?' 또는 '얼마나 좋아해'라고 확인하는 일이 많다. 이들은 사랑에 너무 많은 것을 기대하기 때문에 헤어져 있을 때 견디지 못하고 상대가 다른 이성에게 조금만 관심을 갖게 되면 심한 질투심을 느낀다.";
    $scope.result.A = {};
    $scope.result.A.max = 9;
    $scope.result.A.head = "헌신적 사랑(Selfless Love)";
    $scope.result.A.body = "소위 아가페적인 사랑이 헌신적인 사랑이다. 자기자신의 욕구보다 사랑하는 사람을 더 배려하며 사랑이란 베푸는 것이지 받는 것이 아니라고 생각한다. 따라서 상대가 자신에게 실망을 시키거나 배신한 경우마저도 실망하지 않고 자비심을 베풀며 무시하는 학대하는 상대에게도 인내심을 갖고 헌신한다. 고난과 역경에서도 인내심을 발휘한다. 상대가 떠나도 오랜 시간 동안 돌아오기를 기대하면서 참아낸다. 철저한 자기통제 이면에는 변화에 대한 두려움과 열등감이 숨겨진 경우들이 있다.";
    $scope.result.L = {};
    $scope.result.L.max = 10;
    $scope.result.L.head = "논리적 사랑(Logical Love)";
    $scope.result.L.body = "짝을 찾을 때 경제수준이나, 학력, 가정환경, 외모 등 차후에 갈등의 소지가 될 수 있는 요인들을 사전에 고려하는 사람들이 여기에 속한다. 이 사람들은 현실적이고 이성적 사랑을 추구하기 때문에 관계가 장기적으로 지속되기 어려운 대상은 처음부터 포기한다. 상대방과 자신의 장단점을 고려하고 사랑이란 일종의 공정성에 기반을 둔 거래라고 생각하기 때문에 책임질 수 없는 불장난을 하지는 않는 경향이 있다. 사랑을 시작할 때와 마찬가지로 끝낼 때도 서로 상처받지 않고 결별할 수 있는 방법들을 모색하는 경향이 있다.";
    $scope.result.P = {};
    $scope.result.P.max = 7;
    $scope.result.P.head = "유희적 사랑(Game-Playing Love)";
    $scope.result.P.body = "유희적 사랑을 추구하는 사람은 사랑을 책임져야 하는 것으로 보기보다는 인생을 즐기기 위한 하나의 수단으로 생각한다. 여러 가지 취미생활을 할 수 있는 것과 마찬가지로 파트너도 한 사람으로 만족하지 못하는 경향이 있다. 사랑도 게임처럼 재미있게 할 수 있어야 되기 때문에 책임감이나 의무감에 많은 비중을 두지 않는다. 섹스를 추구하기 때문에 다양하고 노련한 취향과 매너를 보여줄 수는 있으나 지나치게 친밀해지는 것을 경계하고 사랑하는 사람과 헤어져도 심한 고통을 겪지 않는다.";
